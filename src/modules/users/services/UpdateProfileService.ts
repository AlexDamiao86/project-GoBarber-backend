...
class UpdateProfileService {
  ...
  public async execute({
    ...
    if (checkExistingEmail && checkExistingEmail.id !== user.id) {
    ...
    if (password && !old_password) {
      throw new AppError('Previous password must be informed');
    }
    ...
    if (password && old_password) {
      ...
      if (!comparePasswords) {
        throw new AppError('Previous password is invalid');
      }
